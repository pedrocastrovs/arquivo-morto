  -- =============================================================================
  -- CORREÇÃO: "new row violates row-level security policy for table locations"
  -- =============================================================================
  -- Execute no Supabase → SQL Editor (projeto arquivo-morto)
  --
  -- Causa: políticas antigas só permitiam administrador inserir em locations.
  -- Este script libera operador + administrador na estrutura física.
  -- =============================================================================

  -- 1) Confirme seu papel (substitua o e-mail e rode se precisar):
  -- update public.profiles set role = 'administrador' where email = 'seu@email.com';

  -- 2) Políticas de escrita (idempotente)
  ...
  drop policy if exists "sectors_write_operador" on public.sectors;
  drop policy if exists "sectors_update_operador" on public.sectors;
  drop policy if exists "sectors_delete_operador" on public.sectors;
  drop policy if exists "units_write_operador" on public.units;
  drop policy if exists "units_update_operador" on public.units;
  drop policy if exists "units_delete_operador" on public.units;
  drop policy if exists "locations_write_operador" on public.locations;
  drop policy if exists "locations_update_operador" on public.locations;
  drop policy if exists "locations_delete_operador" on public.locations;
  drop policy if exists "streets_write_operador" on public.streets;
  drop policy if exists "streets_update_operador" on public.streets;
  drop policy if exists "streets_delete_operador" on public.streets;
  drop policy if exists "buildings_write_operador" on public.buildings;
  drop policy if exists "buildings_update_operador" on public.buildings;
  drop policy if exists "buildings_delete_operador" on public.buildings;
  drop policy if exists "floors_write_operador" on public.floors;
  drop policy if exists "floors_update_operador" on public.floors;
  drop policy if exists "floors_delete_operador" on public.floors;
  drop policy if exists "towers_write_operador" on public.towers;
  drop policy if exists "towers_update_operador" on public.towers;
  drop policy if exists "towers_delete_operador" on public.towers;
  drop policy if exists "positions_write_operador" on public.positions;
  drop policy if exists "positions_update_operador" on public.positions;
  drop policy if exists "positions_delete_operador" on public.positions;
